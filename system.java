...
public class system {
    ...
    public patient patient_details(String l_name, int SSN) {
        ...
        for (patient p : p_list) {
            if (p.l_name.toLowerCase().equals(l_name) && p.SSN == SSN) return p;
        }
    ...
    public boolean add_date(String date, String time, patient p) {

        String s = date + "  " + time;

        if (date_list.containsKey(s)) return false;
        else if (lookupDateMap.containsKey(p)) return false;
